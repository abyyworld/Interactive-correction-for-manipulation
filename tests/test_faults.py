...
@pytest.mark.parametrize("fault_name", [
    "grasp_offset", "premature_close", "weak_grip", "lift_slip", "early_release", "wrong_object",
])
def test_every_fault_fires_and_causes_failure(env, fault_name):
    ...
    n = 10
    for i in range(n):
        injector = FaultInjector(FaultSpec(type=FaultType(fault_name), severity=0.9),
...
def test_weak_grip_does_not_leak_into_the_next_episode(env):
    ...
    injector = FaultInjector(FaultSpec(type=FaultType.WEAK_GRIP, severity=1.0),
