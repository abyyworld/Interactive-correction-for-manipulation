...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.sweep:
        ...
        if not args.quiet:
            ...
            for acc, rep in results.items():
                ...
                print(f"{acc:<11}{d.get('onset_misattribution_rate', float('nan')):>9.3f}"
    ...
    if args.report:
        ...
        summary = analyse(episodes, n_phases=4,
                          phase_names=("approach", "grasp", "lift", "place"))
        html = build_report(summary, episodes, title="Error attribution study",
...
def _print_report(report: dict) -> None:
    ...
    for lag, v in report["by_lag_class"].items():
        print(f"{lag:<15}{v['n']:>5.0f}{v['onset_misattribution_rate']:>9.3f}"
    ...
    for k, v in report["per_fault"].items():
        print(f"{k:<18}{v['detection_rate']:>8.2f}{v['success_rate_with_correction']:>9.2f}"
